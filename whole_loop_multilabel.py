...
if args.testing:
    data = pd.read_csv('./data/multi-label.csv').sample(500)
    train, test = train_test_split(data, test_size=0.2, stratify=data['identity_hate'])
    test, validation = train_test_split(data, test_size=0.5, stratify=data['identity_hate'])

elif args.running:
    data = pd.read_csv('./data/multi-label.csv')
    print('data numbers:', len(data))
    train, test = train_test_split(data, test_size=0.2, stratify=data['identity_hate'])
    test, validation = train_test_split(data, test_size=0.5, stratify=data['identity_hate'])
else:
    print('need to define parameter, it is "--running" or "--testing"')

print('train data numbers', len(train))
print('test data numbers', len(test))
print('validation data numbers', len(validation))
